...
namespace ReservationSystem2022.Middleware
{
    ...
    public class UserAuthenticationService : IUserAuthenticationService
    {
        ...
        public async Task<bool> IsAllowed(string username, ItemDTO item)
        {
            ...
            if (dbItem==null && item.Owner==user.UserName)//jos lisätään uutta itemiä
            {
                return true;
            }
            if (dbItem == null)
            ...
            if (user.Id== dbItem.Owner.Id) //editointi tai delete
            {
                return true;
            }
            return false;
        ...
        {
            User? dbUser = await _context.Users.Where(x => x.UserName == username).FirstOrDefaultAsync();
        ...
        {
            User? user = await _context.Users.Where(x => x.UserName == username).FirstOrDefaultAsync();
            Reservation? dbReservation =   _context.Reservations.Include(i=> i.Owner).FirstOrDefault(i => i.Id==reservation.Id);

            if (user == null)
            {
                return false;
            }
            if (dbReservation == null && reservation.Owner == user.UserName)//jos lisätään uutta itemiä
            {
                return true;
            }
